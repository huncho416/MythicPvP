...
class TabListManager(private val radium: Radium) {
    ...
    private fun parseColoredText(text: String): Component {
        return if (text.isBlank()) {
            ...
            // Use legacy section (§) serializer instead of ampersand (&) for better compatibility
            LegacyComponentSerializer.legacySection().deserialize(text.replace('&', '§'))
        }
    }
                        ...
                        if (canSeePlayer) {
                            ...
                            if (tabEntry.isPresent) {
                                val finalDisplayName = if (isVanished) {
                                    Component.text()
                                        .append(radium.yamlFactory.getMessageComponent("vanish.tablist_indicator"))
                                ...
                                radium.logger.debug("Updated tab display for ${player.username} (vanished: $isVanished)")
                            }
                        } else {
